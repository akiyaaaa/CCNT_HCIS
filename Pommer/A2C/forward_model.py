...
class ForwardModel(object):
    ...
    def get_rewards(agents, game_type, step_count, max_steps):
        ...
        if game_type == constants.GameType.FFA:
            pick_up_rewards = np.array([0.01 * int(agent.ammo) + 0.01 * int(agent.blast_strength) +0.05 *int(agent.can_kick) for agent in agents])
            if len(alive_agents) == 1:
                # An agent won. Give them +1, others -1.
                end_rewards = [2 * int(agent.is_alive) - 1 for agent in agents]
            elif step_count >= max_steps:
                # Game is over from time. Everyone gets -1.
                end_rewards = [-1] * 4
            else:
                # Game running: 0 for alive, -1 for dead.
                end_rewards = [int(agent.is_alive) - 1 for agent in agents]
            total_rewards = end_rewards + pick_up_rewards
            return total_rewards
        elif game_type == constants.GameType.OneVsOne:
            pick_up_rewards = np.array(
                [0.01 * int(agent.ammo) + 0.01 * int(agent.blast_strength) + 0.05 * int(agent.can_kick) for agent in
                 agents])
            if len(alive_agents) == 1:
                # An agent won. Give them +1, the other -1.
                endrewards = [2 * int(agent.is_alive) - 1 for agent in agents]
            elif step_count >= max_steps:
                # Game is over from time. Everyone gets -1.
                endrewards = [-1] * 2
            else:
                # Game running
                endrewards = [0, 0]
            total_rewards = endrewards + pick_up_rewards
            return total_rewards
        else:
            # We are playing a team game.
