...
class World():   # 定义 world
    def __init__(self, init_gmodel = True):
        if init_gmodel: 
            self.gmodel = A2CNet(gpu = True) # Global model 定义模型
...
def unroll_rollouts(gmodel, list_of_full_rollouts):
    ...
    for (s, a, r, d, h, p, v) in list_of_full_rollouts:
        ...
        for i in reversed(range(len(r))):
            # print("len:", len(r), "reward:", rewards)
...
def train(world):
    ...
    for i in range(40000):
        ...
        with open("training.txt", "a") as f: print(last_rewards, "\t", rr, "\t", round(gmodel.gamma,4), "\t", round(vl,3),"\t", round(pl,3),"\t", round(l,3), file=f)
